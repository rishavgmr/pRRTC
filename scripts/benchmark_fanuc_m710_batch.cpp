// Dev-only BATCH benchmark driver (RSW-2740 Phase 3): sibling of benchmark_fanuc_m710.cpp,
// same env/tool/robot-override/joint-limit loading and the same SDF-based collision-checking
// (see that file's own header comment for the details), but solves a whole batch of (start,
// goal) pairs from this case's problems_batch.json CONCURRENTLY (one std::thread per pair, each
// solved once) instead of repeatedly re-solving a single fixed pair from problem.json. Verifies
// pRRTC's solve() reentrancy fix (RSW-2740 Phase 2: rrtc()'s __device__ globals -> per-call
// pointers) under real concurrent load and reports the actual speedup over solving the same
// batch serially, not just correctness.
//
// Not part of any test suite - build via the benchmark_fanuc_m710_batch CMake
// target and run by hand.
...
int main() {
    ...
    std::vector<Configuration> starts(num_problems);
    std::vector<std::vector<Configuration>> goals_per_problem(num_problems);
    for (std::size_t i = 0; i < num_problems; ++i) {
        starts[i] = config_from_json<Configuration>(problems[i]["start"]);
        goals_per_problem[i] = {config_from_json<Configuration>(problems[i]["goal"])};
    }
    std::cout << "Loaded " << num_problems << " problems from problems_batch.json\n";

    // Uploaded ONCE before any solve() call, same reasoning as benchmark_fanuc_m710.cpp: solve()
    // no longer resets the device between calls (RSW-2740 Phase 2), so nothing wipes this
    // __constant__ state out from under any of the concurrent threads below.
    ...
    // Shared by both passes below - solves one problem and records it into whichever results
    // vector/phase label the caller passes. The concurrent pass calls this from N threads at
    // once; the serial baseline pass calls it N times in a row on the main thread. Same logic,
    // so the only difference between the two passes is real: contention or none.
    auto solve_one = [&](std::size_t idx, std::vector<nlohmann::json>& out, const char* phase) {
        Configuration start = starts[idx];
        std::vector<Configuration> goals = goals_per_problem[idx];
        Environment<float> local_env{};

        auto t0 = std::chrono::steady_clock::now();
        auto result = pRRTC::solve<Robot>(start, goals, local_env, settings);
        ...
        run["goal_config"] = config_to_json(goals[0]);
        if (result.solved) {
...

        {
            ...
            std::cout << "[" << phase << "] problem " << idx << ": solved=" << result.solved
    ...
    // however much the N kernels contended with each other on this one GPU (no MPS) - their sum
    // is NOT a valid estimate of "time to solve these serially". To get an honest speedup number
    // at any batch size, actually solve the same problems one at a time here, each with the GPU
    // to itself, and compare that real total against batch_wall_time_s.
    ...
    // batch one at a time. >1 means interleaving them is worth it; <=1 means this GPU is already
    // saturated at this batch size and running them sequentially would have been as fast or
    // faster - exactly the question this pass exists to answer.
    ...
    output["success_rate_concurrent"] = static_cast<double>(num_success_concurrent) / num_problems;
    output["success_rate_serial"] = static_cast<double>(num_success_serial) / num_problems;
    ...
    const std::string output_path = output_dir + "/prrtc_batch_benchmark_" + std::to_string(now_s) + ".json";
